...
void clearify(Node& a1)//将a1中系数为0的项去掉
{
...
void calculate(stack<Node> &OPND, char op)
{
	Node a2 = OPND.top();
	OPND.pop();
	Node a1 = OPND.top();
		...
		for (int i = 0; i < a2.pos; i++)
		{
			...
			else
			{
				for (int j = 0; j < a1.pos; j++)
				{
					...
					p[1] = '\0';
					temp.poly[j]=copy.poly[j];
					strcat_s(temp.poly[j],resultsize,p);
					strcat_s(temp.poly[j],resultsize,a2.poly[i]);
...
void output(Node result)
{
	...
	for (int i = maxindex; i > 0; i--)
	{
		...
		if (k == 1)
		{
			printf("\"%.1f*%s\",", result.coef[position[0]], result.poly[position[0]]);
		}
		if (k > 1)
		{
			for(int i=0;i<k;i++)
...
int main()
{
	...
	gets_s(expression,MAXSIZE);
	expression[strlen(expression)+1] = '\0';
	expression[strlen(expression) ] = ';';
	char temp1[] = { 'a','b','f','\0'};
	char temp2[] = { 'b','d','\0'};
	char* evalvars[] = {temp1,temp2};
	char temp3[] = { '1','\0'};
	char temp4[] = { '2','\0'};
	char *evalints[] = { temp3,temp4,};
	int num = 2;//求值映射元素个数
